...
public sealed class SessionController : JsonAPIController
{
			...
			case "authorization_code":
				{
					if (code != null)
					{
						Code? codeAuth = await codeService.TakeCodeAsync(CodeKind.Authorization, code);
						if (codeAuth != null)
						{
			...
			case "exchange_code":
				{
					if (exchangeCode != null)
					{
						...
						Code? codeExchange = await codeService.TakeCodeAsync(CodeKind.Exchange, exchangeCode);
	...
	public async Task<IActionResult> CreateExchangeCode()
	{
		...
		*/
		...
		Code? code = await codeService.CreateCodeAsync(CodeKind.Exchange, user.Session.AccountID, user.Session.ClientID);
		if (code == null)
		{
			return BadRequest(new ErrorResponse()
			{
	...
	public async Task<IActionResult> CreateAuthorizationCode()
	{
		...
		Code? authCode = await codeService.CreateCodeAsync(CodeKind.Authorization, user.Session.AccountID, user.Session.ClientID);
		if (authCode == null)
		{
			return BadRequest();
		}
